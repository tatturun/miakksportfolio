...
import { FaGithub, FaTwitter, FaGlobe, FaPenNib, FaExternalLinkAlt, FaCertificate } from "react-icons/fa";
          ...
          {member.socials && (
            <div className="flex gap-3 mt-4 md:absolute md:bottom-6 md:right-8 justify-center w-full md:w-auto">
              {member.socials.github && (
                <SocialLink href={member.socials.github} icon={<FaGithub />} />
              )}
              {member.socials.twitter && (
                <SocialLink href={member.socials.twitter} icon={<FaTwitter />} />
              )}
              {member.socials.qiita && (
                <SocialLink href={member.socials.qiita} icon={<FaPenNib />} />
              )}
              {member.socials.portfolio && (
                <SocialLink href={member.socials.portfolio} icon={<FaGlobe />} />
              )}
            </div>
          )}
        </div>
          ...
          <div className="space-y-6 flex flex-col h-full">
            ...
            <section className="bg-white rounded-2xl shadow-sm p-8 border border-slate-100">
              <h2 className="text-lg font-bold text-slate-800 mb-6 flex items-center">
                <span className="w-1.5 h-5 rounded-full mr-3" style={{ backgroundColor: member.color }}></span>
                Skills
              </h2>
            ...
            <section className="bg-white rounded-2xl shadow-sm p-8 border border-slate-100 flex-grow">
              ...
              <div className="space-y-4">
                ...
                {(!member.certifications || member.certifications.length === 0) && (
                  <p className="text-slate-400 text-sm">No certifications listed yet.</p>
                )}
              </div>
            </section>
            ...
            <div className="space-y-6">
              {member.history?.map((item, index) => (
                <div key={index} className="flex gap-4 items-start relative">
                  ...
                  <div className="flex-1"> {/* flex-1を追加して幅を確保 */}
              ...
              {(!member.history || member.history.length === 0) && (
                <p className="text-slate-400 text-sm">No history listed yet.</p>
              )}
            </div>
          </section>
        </div>
